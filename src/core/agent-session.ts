...
import {
  ...
  CHECKPOINT_SUMMARY_START_RATIO,
  CHECKPOINT_SUMMARY_STOP_RATIO,
  CheckpointCandidate,
...
export class AgentSession {
  ...
  constructor(
    ...
    this.turnController = new AgentTurnController({
      ...
      checkpointCompactionCoordinator: this.useCheckpointCompaction && this.useCheckpointCandidates
  ...
  private async handleInput(
    ...
    return this.withLogContext(async () => {
      ...
      try {
        const messagesBeforeCompaction = stripAssistantArtifactsFromMessages(this.messages);
        const checkpointRevisionBeforeBoundary = this.checkpointRevision;
        const messagesForCompaction = await this.handleCheckpointCandidateBoundary(
          messagesBeforeCompaction,
          lifecycleGeneration,
          'pre_turn',
        );
        // Candidate mode owns the pre-turn boundary: it either commits a ready
        // candidate, waits/falls back at the stop point, or leaves the transcript
        // untouched below the stop point. Running the synchronous coordinator
        // again here duplicates that decision and can create a second summary.
        const compactionResult = this.useCheckpointCompaction
          && this.useCheckpointCandidates
          && this.checkpointRevision !== checkpointRevisionBeforeBoundary
  ...
  private async handleCheckpointCandidateBoundary(
    ...
    if (stopPointReached) {
      const fallback = await this.runSerialCheckpointFallback(messagesBeforeCompaction, phase);
